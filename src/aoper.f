...
C       with rotations about stability axes 
        IF(LSOL) THEN
         CALL GETFILE(LU,COMARG)
C
         IF(LU.LE.-1) THEN
...
C------ stability derivatives for forces, rotations in stability axes 
C       with moments about body axes 
        IF(LSOL) THEN
         CALL GETFILE(LU,COMARG)
C
         IF(LU.LE.-1) THEN
...
C------ stability derivatives for forces, moments in body axes
C       with rotations about body axes 
        IF(LSOL) THEN
         CALL GETFILE(LU,COMARG)
C
         IF(LU.LE.-1) THEN
...
C
        IF(USEMRF) THEN
         IF(LPTOT)   CALL MRFTOT(LUOUT, 'TOT')
         IF(LPSURF)  CALL MRFSURF(LUOUT)
         IF(LPSTRP)  CALL MRFSTRP(LUOUT)
         IF(LPELE)   CALL MRFELE(LUOUT)
        ELSE
...
C
C------------------------------------------------------
      ELSE IF(COMAND.EQ.'MRF ') THEN
C------ all subsequent data file writes are full-precision machine readable format
        USEMRF = .TRUE.
        WRITE(*,'(A,A)')
     &    'MRF: all subsequent output in full-precision ',
     &    'machine readable format'
cc#ifdef USE_CPOML
C
C------------------------------------------------------
      ELSE IF(COMAND.EQ.'CPOM') THEN
C------ write OML surface pressures to a file
        CALL CPOML()
cc#endif
C
C------------------------------------------------------
